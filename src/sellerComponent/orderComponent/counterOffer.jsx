import React, { useEffect, useState } from "react";
import { Link } from "react-router-dom";
import {
  getOrderDetails,
  sendCounterOffer,
} from "../../apiServices/sellerApiHandler/orderApiHandler";
import Header from "../commonComponent/header";
import { useParams } from "react-router-dom";
import moment from "moment";
import { useNavigate } from "react-router-dom";

const CounterOffer = () => {
  const [data, setData] = useState("");

  let { id } = useParams();
  const navigate = useNavigate();

  useEffect(() => {
    getOrDetails();
  }, []);

  const getOrDetails = async () => {
    const { data } = await getOrderDetails(id);
    if (!data.error) {
      setData(data.results.order);
    }
  };
  const onChange = async (type, e, index) => {
    let product = data;

    if (type === "date") {
      product.pick_up_date = new Date(document.getElementById("date").value);
      console.log(document.getElementById("date").value);
    } else if (type === "time") {
      product.pick_up_time = document.getElementById("time").value;
      console.log(document.getElementById("time").value);
    } else if (type === "payment") {
      product.payment = e.target.value;
      console.log(e.target.value, " hello");
    } else {
      product.product[index].quantity = e.target.value;
      console.log(e.target.value, " hello");
    }
    setData({ ...product });
  };

  const deleteCounterOffer = async (id) => {
    let product = data;
    product.product = product.product.filter(
      (item) => String(item.productId) !== String(id)
    );
    setData(product);
    console.log(product, id);
  };

  const sendOffer = async () => {
    const response = await sendCounterOffer(data);
    if (!response.data.error) {
      navigate("/send-counter-offer");
    }
  };
  return (
    <>
      <Header />
      <section class="similler_order">
        <div class="container">
          <div class="row py-3 align-items-start">
            <div class="col-md-auto mb-md-0 mb-2">
              <Link
                class="product_comman_btn m-0"
                to={`/view-new-orders/${id}`}
              >
                <img src="../assets/img/arrow-left-line.png" alt="Back" />
                Back
              </Link>
            </div>
            <div class="col">
              <div class="order_top_mains">
                <div class="row">
                  <div class="col-xl-3 col-lg-4 col-md-6 my-md-2 my-1">
                    <div class="row order_top_box align-items-center">
                      <div class="col-5">
                        <strong>Order Date</strong>
                      </div>
                      <div class="col-auto px-0">
                        <span class="text-white">:</span>
                      </div>
                      <div class="col pe-0">
                        <span>{moment(data.createdAt).format("LL")}</span>
                      </div>
                    </div>
                  </div>
                  <div class="col-xl-3 col-lg-4 col-md-6 my-md-2 my-1">
                    <div class="row order_top_box align-items-center">
                      <div class="col-5">
                        <strong>Buyer</strong>
                      </div>
                      <div class="col-auto px-0">
                        <span class="text-white">:</span>
                      </div>
                      <div class="col pe-0">
                        <span>{data.buyer?.business_trading_name}</span>
                      </div>
                    </div>
                  </div>
                  <div class="col-xl-3 col-lg-4 col-md-6 my-md-2 my-1">
                    <div class="row order_top_box align-items-center">
                      <div class="col-5">
                        <strong>Phone</strong>
                      </div>
                      <div class="col-auto px-0">
                        <span class="text-white">:</span>
                      </div>
                      <div class="col pe-0">
                        <span>{data.phone_number}</span>
                      </div>
                    </div>
                  </div>
                  <div class="col-xl-3 col-lg-4 col-md-6 my-md-2 my-1">
                    <div class="row order_top_box align-items-center">
                      <div class="col-5">
                        <strong>Notes</strong>
                      </div>
                      <div class="col-auto px-0">
                        <span class="text-white">:</span>
                      </div>
                      <div class="col pe-0">
                        <span>{data.notes}</span>
                      </div>
                    </div>
                  </div>
                  <div class="col-xl-3 col-lg-4 col-md-6 my-md-2 my-1">
                    <div class="row order_top_box align-items-center">
                      <div class="col-5">
                        <strong>Pickup Date</strong>
                      </div>
                      <div class="col-auto px-0">
                        <span class="text-white">:</span>
                      </div>
                      <div class="col-6 pe-0 d-flex">
                        {console.log(data)}
                        <span>
                          {moment(data.pick_up_date).format(
                            "ddd, MMMM Do YYYY"
                          )}
                        </span>

                        <input
                          class="form-control form-control-date"
                          type="date"
                          id="date"
                          value=""
                          onChange={() => onChange("date")}
                        />
                      </div>
                    </div>
                  </div>
                  <div class="col-xl-3 col-lg-4 col-md-6 my-md-2 my-1">
                    <div class="row order_top_box align-items-center">
                      <div class="col-5">
                        <strong>Pickup Time</strong>
                      </div>
                      <div class="col-auto px-0">
                        <span class="text-white">:</span>
                      </div>
                      <div class="col-6 pe-0 d-flex">
                        <span className="mt-1">{data.pick_up_time}</span>

                        <input
                          class="form-control form-control-date2"
                          type="time"
                          id="time"
                          value=""
                          onChange={() => onChange("time")}
                        />
                      </div>
                    </div>
                  </div>
                  <div class="col-xl-3 col-lg-4 col-md-6 my-md-2 my-1">
                    <div class="row order_top_box align-items-center">
                      <div class="col-5">
                        <strong>Trans Type</strong>
                      </div>
                      <div class="col-auto px-0">
                        <span class="text-white">:</span>
                      </div>
                      <div class="col pe-0 d-flex">
                        <span className="mt-1">{data.payment}</span>

                        <select
                          name="payment"
                          id="payment"
                          class="form-control-date3"
                          onChange={async (e) => await onChange("payment", e)}
                        >
                          <option value="SMCS">SMCS</option>
                          <option value="INVOICE">INVOICE</option>
                          <option value="CASH">CASH</option>
                          <option value="CARD">CARD</option>
                        </select>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div class="container-fluid px-0">
          <div class="similler_order_table comman_table_design">
            <div class="table-responsive">
              <table class="table mb-0">
                <thead>
                  <tr style={{ background: "#374251" }}>
                    <th>Variety</th>
                    <th>Type</th>
                    <th>Unit</th>
                    <th>Quantity</th>
                    <th>Delete Product</th>
                  </tr>
                </thead>
                <tbody>
                  {data.product?.map((item, index) => {
                    return (
                      <tr>
                        <td>{item.product.variety.variety}</td>
                        <td>{item.product.type.type}</td>
                        <td>{item.product.units.unit}</td>

                        <td>
                          <input
                            class="consiment_data_box"
                            type="text"
                            defaultValue={item.quantity}
                            id="quantity"
                            onChange={(e) => onChange("quantity", e, index)}
                          />
                        </td>
                        <td>
                          <Link
                            class="icon_design"
                            to=""
                            onClick={() => deleteCounterOffer(item.productId)}
                          >
                            <img src="../assets/img/delete_white.png" alt="" />
                          </Link>
                        </td>
                      </tr>
                    );
                  })}
                </tbody>
              </table>
            </div>
            /
          </div>

          <div class="row justify-content-center py-4">
            <div class="col-auto text-center">
              <Link
                class="customer_btn mx-md-3 mb-md-0 mb-2 tables_green_btn"
                to=""
                onClick={() => sendOffer()}
              >
                Send Offer
              </Link>
            </div>
          </div>
        </div>
      </section>
    </>
  );
};

export default CounterOffer;
